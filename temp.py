...
# Load tokenizer and model (example: LLaMA2 or other models from Hugging Face)
MODEL_NAME = "meta-llama/Llama-2-7b-chat-hf"  # Replace with the desired open-source model
...
if uploaded_file is not None:
    st.markdown('<h8 style="color: lightgreen;text-align: center;">File uploaded successfully!</h8>',
...
if submit_button:
    ...
    if uploaded_file is not None:
        ...
        response_text = generate_response_from_llm(input_prompt_template.format(text=resume_text, job_description=job_description))
        print(response_text)
        ...
        if match_percentage >= 80:
            st.markdown('<p style="color: green;font-size: 20px;text-align: left;">Move forward with hiring</p>',
                        unsafe_allow_html=True)


        else:
            st.markdown('<p style="color: lightgreen;font-size: 20px;text-align: left;">Not Selected!</p>',
                        unsafe_allow_html=True)
        ...
        st.text(" \n")
        st.text(" \n")
        st.markdown('<h6 style="color: red;text-align: center;">Please upload your Resume!</h6>',
                    unsafe_allow_html=True)
    ...
    if uploaded_file is not None:
        ...
        st.write(
            f"<div style='text-align:left; font-family: sans-serif; font-size: 20px;'>{match_percentage_str}</div>",
            unsafe_allow_html=True)
    else:
        st.text(" \n")
        st.text(" \n")
...
<div class="footer">
<p>Developed with ❤ by<a style='display: block; text-align: center;' href="https://github.com/Amanpandey3008/ResumeGenie" target="_blank">Aman Pandey</a></p>
</div>
"""
